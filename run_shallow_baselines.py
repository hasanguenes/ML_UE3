...
Usage: 
  python run_shallow_baselines.py                          # Interactive mode
  python run_shallow_baselines.py -d cifar10 -m both -f both -y      # Full CLI mode
  python run_shallow_baselines.py -d gtsrb --train-samples 500 --test-samples 100 -y
  python run_shallow_baselines.py -d cifar10 -m lr -f color --batch  # Batch mode (no prompts)

Options:
  -d, --dataset       Dataset: cifar10, gtsrb
  -m, --model         Model: lr, rf, both
  -f, --features      Features: color, sift, both
  --train-samples     Number of train samples (default: full dataset)
  --test-samples      Number of test samples (default: full dataset)
  --cv-folds          Number of CV folds (default: 3)
  --bins              Color histogram bins (default: 8)
  --clusters          BoVW clusters (default: 100)
  -y, --yes           Skip confirmation prompt
  --batch             Batch mode: no interactive prompts, fails if -d/-m/-f missing
"""
import argparse
import sys
import numpy as np
...
def save_features_as_parquet(X_train: np.ndarray, y_train: np.ndarray,
                              X_test: np.ndarray, y_test: np.ndarray,
                              feature_name: str, base_dir: Path):
    """Save feature arrays as parquet files."""
    features_dir = base_dir / "features" / feature_name
    features_dir.mkdir(parents=True, exist_ok=True)
    
    # Create DataFrames with feature columns and label
    ...
    print(f"  ✓ Saved features to parquet:")
...
def extract_color_histogram_3d(Xtr: np.ndarray, Xte: np.ndarray, 
    ...
    from feature_extractions import hist_3d_features
    
    # Train extraction
    print(f"  Processing TRAIN set ({len(Xtr)} images)...")
    train_start = time.time()
    X_train_feat = hist_3d_features(Xtr, bins=bins)
    train_time = time.time() - train_start
    
    # Test extraction
    print(f"  Processing TEST set ({len(Xte)} images)...")
    test_start = time.time()
    X_test_feat = hist_3d_features(Xte, bins=bins)
...
def extract_sift_bovw(Xtr: np.ndarray, Xte: np.ndarray, 
    ...
    from feature_extractions import BOV
...
def train_logistic_regression(X_train: np.ndarray, y_train: np.ndarray,
    ...
    joblib.dump(best_penalty_result['model'], model_path)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='Shallow ML Baselines for Image Classification',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python run_shallow_baselines.py                                  # Interactive mode
  python run_shallow_baselines.py -d cifar10 -m both -f both -y    # Full CLI, skip confirmation
  python run_shallow_baselines.py -d cifar10 -m lr -f color --batch  # Batch mode (no prompts at all)
  python run_shallow_baselines.py -d gtsrb -m lr -f color -y       # GTSRB, LR only, ColorHist only
  python run_shallow_baselines.py -d cifar10 --train-samples 500 --test-samples 100 -y  # Quick test
  python run_shallow_baselines.py --cv-folds 5 --bins 16           # Custom CV folds and bins
        """
    )
    parser.add_argument('-d', '--dataset', choices=['cifar10', 'gtsrb'],
                        help='Dataset to use')
    parser.add_argument('-m', '--model', choices=['lr', 'rf', 'both'],
                        help='Model to train: lr=Logistic Regression, rf=Random Forest, both=Both')
    parser.add_argument('-f', '--features', choices=['color', 'sift', 'both'],
                        help='Features to extract: color=ColorHistogram, sift=SIFT+BoVW, both=Both')
    parser.add_argument('--train-samples', type=int, default=None,
                        help='Number of train samples (default: full dataset)')
    parser.add_argument('--test-samples', type=int, default=None,
                        help='Number of test samples (default: full dataset)')
    parser.add_argument('--cv-folds', type=int, default=None,
                        help=f'Number of CV folds (default: {CV_FOLDS})')
    parser.add_argument('--bins', type=int, default=None,
                        help='Color histogram bins (default: 8)')
    parser.add_argument('--clusters', type=int, default=None,
                        help=f'Number of BoVW clusters (default: {BOVW_N_CLUSTERS})')
    parser.add_argument('-y', '--yes', action='store_true',
                        help='Skip confirmation prompt')
    parser.add_argument('--batch', action='store_true',
                        help='Batch mode: fail if required args (-d, -m, -f) are missing (no interactive prompts)')
    return parser.parse_args()

def main():
    global MAX_TRAIN_SAMPLES, MAX_TEST_SAMPLES, CV_FOLDS, COLOR_HIST_BINS, BOVW_N_CLUSTERS
    
    args = parse_args()
    
    # Batch mode: check required arguments
    if args.batch:
        missing = []
        if not args.dataset:
            missing.append('--dataset / -d')
        if not args.model:
            missing.append('--model / -m')
        if not args.features:
            missing.append('--features / -f')
        if missing:
            print(f"Error: Batch mode requires the following arguments: {', '.join(missing)}")
            print("Run with --help for usage information.")
            sys.exit(1)
        # Batch mode implies --yes
        args.yes = True
    ...
    # Model selection
    if args.model:
        model_choice = {'lr': 1, 'rf': 2, 'both': 3}[args.model]
        print(f"Model: {args.model} (from CLI)")
    else:
        print("\nSelect model:")
    ...
    # Feature selection
    if args.features:
        feature_choice = {'color': 1, 'sift': 2, 'both': 3}[args.features]
        print(f"Features: {args.features} (from CLI)")
    else:
        print("\nSelect features:")
    ...
    if args.train_samples is not None or args.test_samples is not None or args.batch:
        MAX_TRAIN_SAMPLES = args.train_samples
        MAX_TEST_SAMPLES = args.test_samples
        if not args.batch or args.train_samples or args.test_samples:
            print(f"Train samples: {MAX_TRAIN_SAMPLES or 'Full dataset'} (from CLI)")
            print(f"Test samples: {MAX_TEST_SAMPLES or 'Full dataset'} (from CLI)")
    else:
        print("\nSelect number of samples (for quick testing, e.g., 500 train / 100 test):")
        print("  Press Enter or type any letter to use the full dataset.")
        
        train_input = input("Number of train samples: ").strip()
        if train_input.isdigit() and int(train_input) > 0:
            MAX_TRAIN_SAMPLES = int(train_input)
        else:
            MAX_TRAIN_SAMPLES = None
        
        test_input = input("Number of test samples: ").strip()
        if test_input.isdigit() and int(test_input) > 0:
            MAX_TEST_SAMPLES = int(test_input)
        else:
            MAX_TEST_SAMPLES = None

    # Apply optional CLI overrides for hyperparameters
    if args.cv_folds is not None:
        CV_FOLDS = args.cv_folds
    if args.bins is not None:
        COLOR_HIST_BINS = [args.bins]
    if args.clusters is not None:
        BOVW_N_CLUSTERS = args.clusters

    # Confirmation
    bins_info = COLOR_HIST_BINS if isinstance(COLOR_HIST_BINS, list) else [COLOR_HIST_BINS]
    train_samples_str = str(MAX_TRAIN_SAMPLES) if MAX_TRAIN_SAMPLES else "Full dataset"
    test_samples_str = str(MAX_TEST_SAMPLES) if MAX_TEST_SAMPLES else "Full dataset"
    print(f"\nConfiguration:")
    ...
    if feature_choice in [2, 3]:
        print(f"  BoVW clusters: {BOVW_N_CLUSTERS}")
    print(f"  CV Folds: {CV_FOLDS}")
    print(f"  Train samples: {train_samples_str}")
    print(f"  Test samples: {test_samples_str}")

    if args.yes:
        print("\nStarting training... (confirmation skipped with -y)")
    else:
        while True:
    ...
    if feature_choice in [1, 3]:  # Color Histogram
        ...
        for bins in bins_list:
            ...
            save_features_as_parquet(X_train_hist, ytr, X_test_hist, yte, feature_name, base_dir)
    ...
    if feature_choice in [2, 3]:  # SIFT + BoVW
        ...
        # Save features as parquet
        save_features_as_parquet(X_train_bovw, ytr, X_test_bovw, yte, "SIFT_BoVW", base_dir)
    ...
    print(f"Best models (joblib): <model_folder>/best_model.joblib")
    print(f"Features (parquet): features/<feature_name>/train.parquet, test.parquet")
